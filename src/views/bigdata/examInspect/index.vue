<template>
  <div class="app-container calendar-list-container">
    ...
    >
      ...
      <el-table-column align="center" label="巡考情况">
        <template slot-scope="scope">
          <span>{{scope.row.inspectStatus | statusFilter}}</span>
        </template>
      </el-table-column>
      <el-table-column fixed="right" align="center" label="操作" width="150">
        <template slot-scope="scope">
          <el-button
            v-if="bd_examInspect_edit"
            size="small"
            type="success"
            @click="handleUpdate(scope.row)"
          >编辑</el-button>
          <el-button
            v-if="bd_examInspect_del"
            size="small"
            type="danger"
            @click="deleteExamInspect(scope.row)"
          >删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form :model="form" :rules="rules" ref="form" label-width="100px">
        ...
        <el-form-item label="巡考情况" prop="inspectStatus">
          <el-input v-model="form.inspectStatus" placeholder="请输入巡考情况"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        ...
        <el-button
          ...
          type="primary"
          @click="createExamInspect('form')"
        >确 定</el-button>
        <el-button v-else type="primary" @click="updateExamInspect('form')">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
<script>
import {
  getExamInspectsByPage,
  addExamInspect,
  getExamInspect,
  delExamInspect,
  updExamInspect
} from '@/api/bigdata/examInspect'
...
export default {
  ...
  data() {
    return {
      ...
      dialogStatus: '',
      bd_examInspect_add: false,
      bd_examInspect_del: false,
      bd_examInspect_edit: false,
      textMap: {
        update: '编辑',
        create: '创建'
      },
  ...
  created() {
    this.bd_examInspect_add = this.permissions['bd_examInspect_add']
    this.bd_examInspect_edit = this.permissions['bd_examInspect_edit']
    this.bd_examInspect_del = this.permissions['bd_examInspect_del']
  },
  computed: {
  ...
  methods: {
    getList(planId) {
      ...
      getExamInspectsByPage(this.listQuery).then(response => {
    ...
    handleUpdate(row) {
      getExamInspect(row.id).then(response => {
        this.form = response.data
        this.dialogFormVisible = true
        this.dialogStatus = 'update'
      })
    },
    deleteExamInspect(row) {
      this.$confirm('此操作将永久删除数据, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        delExamInspect(row.id).then(() => {
          this.$notify({
            title: '成功',
            message: '删除成功',
            type: 'success',
    ...
    createExamInspect(formName) {
      const set = this.$refs
      set[formName].validate(valid => {
        if (valid) {
          addExamInspect(this.form).then(() => {
    ...
    updateExamInspect(formName) {
      const set = this.$refs
      set[formName].validate(valid => {
        if (valid) {
          this.dialogFormVisible = false
          updExamInspect(this.form).then(() => {
            this.dialogFormVisible = false
            this.getList(this.planId)
            this.$notify({
              title: '成功',
